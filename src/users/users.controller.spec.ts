...
describe('UsersController', () => {
  ...
  it('findAllUsers returs a list of users with the given email', async () =>{
    const users = await controller.findAllUsers('someone@example.com');
    expect(users.length).toEqual(1);
    expect(users[0].email).toEqual('someone@example.com');
  });

  it('findUsers returna s users with the given id', async () =>{
  ...
  it('signin updates session object and returns user', async () => {
    ...
    const user = await controller.signin({email: 'someone@example.com', password:'securepassword123'},
      session
    );
